...
template<typename T>
using TMapVec = std::map<std::string, std::vector<T>>;
...
struct ShadowBorder
{
	...
	ShadowBorder(float left, float top, float right, float bottom);
...
struct SImportParams
{
	...
	ESliceFormat eFormat = ESliceFormat_Ignore;
	ShadowBorder sBorder;
};
